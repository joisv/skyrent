        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-8 mt-6 lg:mt-12">
            ...
            @empty(!$iphones)
                @forelse ($iphones as $iphone)
                    <!-- Card -->
                    <div class="group bg-white rounded-[28px] p-3 shadow-md hover:shadow-xl transition flex flex-col">

                        {{-- Image --}}
                        <div class="flex justify-center items-center mb-4 h-64">
                            <img data-src="{{ asset('storage/' . $iphone->gallery->image) }}" alt="{{ $iphone->name }}"
                                class="lazy max-h-full object-contain group-hover:scale-105 transition duration-300">
                        </div>
                ...
                @empty
                    <p class="text-center col-span-4 text-gray-500" wire:loading.remove>
